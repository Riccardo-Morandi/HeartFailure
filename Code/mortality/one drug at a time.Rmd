...
```{r}
dfAA <- data %>% select(c(SESSO,labelOUT,eta_Min,MCS,PDC_AA))
```
...
```{r}
collect_predictions(rs_AA) %>% group_by(id) %>% roc_curve(labelOUT,.pred_DECEDUTO) %>% autoplot()
```
...
```{r}
dfBB <- data %>% select(c(SESSO,labelOUT,eta_Min,MCS,PDC_BB))
```
...
```{r}
set.seed(123)
dfBB_split <- initial_split(dfBB, strata = labelOUT)
dfBB_train <- training(dfBB_split)
dfBB_test <- testing(dfBB_split)

set.seed(234)
dfBB_folds <- vfold_cv(dfBB_train, strata = labelOUT)
```

lets start by leaving the coefficients as numeric

```{r}
rec_BB <- recipe(labelOUT ~ ., data = dfBB_train) %>% 
  step_normalize(eta_Min) %>% 
  step_cut(starts_with("PDC"),breaks = c(0.001,0.25,0.5,0.75),include_outside_range = FALSE)

wf_BB <- workflow(rec_BB %>% step_downsample(labelOUT),glm_spec)
```
...
```{r}
collect_predictions(rs_BB) %>% group_by(id) %>% roc_curve(labelOUT,.pred_DECEDUTO) %>% autoplot()
```
...
here the results are clearly different, we can see an increase in survival as the PDC increases, and it is better than the referencce which is the patient not taking the BB

##RAS

```{r}
dfRAS <- data %>% select(c(SESSO,labelOUT,eta_Min,MCS,PDC_RAS))
```

```{r}
set.seed(123)
dfRAS_split <- initial_split(dfRAS, strata = labelOUT)
dfRAS_train <- training(dfRAS_split)
dfRAS_test <- testing(dfRAS_split)

set.seed(234)
dfRAS_folds <- vfold_cv(dfRAS_train, strata = labelOUT)
```


```{r}
rec_RAS <- recipe(labelOUT ~ ., data = dfRAS_train) %>% 
  step_normalize(eta_Min) %>% 
  step_cut(starts_with("PDC"),breaks = c(0.001,0.25,0.5,0.75),include_outside_range = FALSE)

wf_RAS <- workflow(rec_RAS %>% step_downsample(labelOUT),glm_spec)
```
...
```{r}
collect_predictions(rs_RAS) %>% group_by(id) %>% roc_curve(labelOUT,.pred_DECEDUTO) %>% autoplot()
```
...
here wee see that the clear difference starts when we have a PDC of more than 0.5
...
```{r}
dfAA <- data %>% filter(PDC_TOT > 0.75) %>% select(c(SESSO,labelOUT,eta_Min,MCS,PDC_AA))
```
...
```{r}
dfBB <- data %>%  filter(PDC_TOT > 0.75) %>% select(c(SESSO,labelOUT,eta_Min,MCS,PDC_BB))
```
...
also fot BB we do not have enough evidence to say that it clearly improves the survival
