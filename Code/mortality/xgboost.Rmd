...
```{r}
rec_xgb <- recipe(labelOUT ~ ., data = df_train) %>% step_dummy(all_nominal_predictors(), one_hot = TRUE)
```
...
```{r}
collect_predictions(xgb_last) %>%  conf_mat(labelOUT,.pred_class)
```
...
```
 this also tells me taht the strongest interaction is with the MCS4
