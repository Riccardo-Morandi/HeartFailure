...
```{r}
rec_basic <- recipe(labelOUT ~ ., data = df_train) %>%  step_dummy(all_nominal_predictors())
```
...
```{r}
collect_predictions(final_rs) %>% conf_mat(labelOUT,.pred_class)
```

we can try to do better by using downsampling

```{r}
rec_dw <- recipe(labelOUT ~ ., data = df_train) %>%  step_dummy(all_nominal_predictors()) %>% step_downsample(labelOUT)
```
...
```{r}
best_tree_spec <- decision_tree(cost_complexity = 0.0000001, tree_depth = 10, min_n = 40 ) %>% 
  set_mode("classification") %>% set_engine("rpart")
```
...
```{r}
collect_predictions(test_rs_dw) %>% conf_mat(labelOUT,.pred_class) 
```

```{r}
collect_predictions(test_rs_dw) %>% roc_curve(labelOUT,.pred_DECEDUTO) %>% autoplot()
```
...
tree_explainer <- explain_tidymodels(
  ...
  data = dplyr::select(df_train, -labelOUT),
  y = as.integer(df_train$labelOUT),
  verbose = TRUE
)
```
...
this does not look very influential, looking at the scale
...
as_tibble(pdp_eta$agr_profiles) %>% 
  ...
  labs(
    ...
    y = "Predicted probability survival",
    color = NULL,
    title = "Partial dependence plot for survival based on eta_Min",
  )
```
...
overall it seems ljust a traslation
...
it sseems just a traslation, no interaction
