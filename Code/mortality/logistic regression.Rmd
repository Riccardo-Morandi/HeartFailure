...
```{r}
ggplot(df,aes(as.factor(MCS))) + geom_bar(aes(fill = labelOUT))
```
...
```{r}
ggplot(df, aes(eta_Min)) + geom_histogram(aes(fill = labelOUT),binwidth = 1)
```
...
```{r}
ggplot(df, aes(PDC_TOT)) + geom_freqpoly(aes(colour = labelOUT),binwidth = 0.01)
```
...
odds_ratios$term <- c("Età", "Sesso M","MCS 1", "MCS 2","MCS 3","MCS 4",
                      "PDC TOT (0.5,0.75]","PDC TOT (0.25,0.5]","PDC TOT (0.75,1]")
```
...
```{r}
P1
ggsave("odds_ratios_pdc_tot.png", dpi = 1000)
```

 the estimates for the PDC in the two classes 0.25-0.5 and 0.5-0.75 is the same we could group them together
 
```{r}
rec2 <- recipe(labelOUT ~ ., data = df_train) %>% 
...
```{r}
collect_predictions(rs2) %>% group_by(id) %>% roc_curve(labelOUT,.pred_DECEDUTO) %>% autoplot()
```
...
we can see that the ODDS ratio relative to the PDC_TOT are high
