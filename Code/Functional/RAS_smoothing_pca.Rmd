---
title: "PDC RSA smoothing and pca"
...
load("~/Documents/Applied Statistics/Project/Materiale_appstat/Code/Functional/PDC_RAS.RData")
...
n=dim(PDC_RAS)[2]
index=sample(n,20)
matplot(PDC_RAS[,index],type = 'l',main='dayly PDC',xlab='day',ylab='PDC')
```
...
data.fd <- Data2fd(y = PDC_RAS,argvals = days,basisobj = basis)
...
```{r}
n <- dim(PDC_RAS)[2]
nbasis <- seq(275,325,25)
gcv <- numeric(length(nbasis))
PDC_RAS_subset <- PDC_RAS[,sample(n,1000)]

for (i in 1:length(nbasis)){
  basis <- create.bspline.basis(rangeval=range(days),nbasis=nbasis[i],norder = 4)
  for(j in 1:1000){
    gcv[i] <- gcv[i] + smooth.basis(days, PDC_RAS_subset[,j], basis)$gcv
  }
  gcv[i] <- gcv[i]/n
}
```
...
since we are interested in doing the pcca let's look at the covariance matrix 

```{r}
basis.1 <- create.bspline.basis(rangeval=range(days),nbasis=365,norder = 4)
data_W.fd.1 <- Data2fd(y = PDC_RAS,argvals = days,basisobj = basis.1)


basis.2 <- create.bspline.basis(rangeval=range(days),nbasis=275,norder = 4)
data_W.fd.2 <- Data2fd(y = PDC_RAS,argvals = days,basisobj = basis.2)
...
data.smooth <- smooth.basis(days, PDC_RAS, functionalPar)
...
plot(media,lwd=2,ylim=c(0,0.7),xlab='days',main='FPC3')
lines(media+pca.res$harmonics[3,]*sqrt(pca.res$values[3]), col=2)
lines(media-pca.res$harmonics[3,]*sqrt(pca.res$values[3]), col=3)
