---
title: "PDC_BB smoothing and pca"
...
load("~/Documents/Applied Statistics/Project/Materiale_appstat/Code/Functional/PDC_BB.RData")
...
n=dim(PDC_BB)[2]
index=sample(n,20)
matplot(PDC_BB[,index],type = 'l',main='dayly PDC',xlab='day',ylab='PDC')
```
...
data.fd <- Data2fd(y = PDC_BB,argvals = days,basisobj = basis)
...
PDC_BB_subset <- PDC_BB[,sample(n,1000)]

for (i in 1:length(nbasis)){
  basis <- create.bspline.basis(rangeval=range(days),nbasis=nbasis[i],norder = 4)
  for(j in 1:1000){
    gcv[i] <- gcv[i] + smooth.basis(days, PDC_BB_subset[,j], basis)$gcv
  }
  gcv[i] <- gcv[i]/n
}
```
...
data_W.fd.1 <- Data2fd(y = PDC_BB,argvals = days,basisobj = basis.1)


basis.2 <- create.bspline.basis(rangeval=range(days),nbasis=275,norder = 4)
data_W.fd.2 <- Data2fd(y = PDC_BB,argvals = days,basisobj = basis.2)
...
the covariance structure is not very similar
...
data.smooth <- smooth.basis(days, PDC_BB, functionalPar)
